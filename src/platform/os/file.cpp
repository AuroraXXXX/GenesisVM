...
void *os::so_load(const char *path_name, char *err_buf, int32_t err_buf_len) {
...
void os::so_unload(void *so) {
    ::dlclose(so);
}

void *os::so_lookup(void *so, const char *name) {
    return ::dlsym(so, name);
}

os::FileType os::stat(const char *path,
                      size_t *file_size,
                      size_t *block_size) {
...
int32_t os::open(const char *path, int32_t fd_flags, size_t trunc_bytes) {
    //检查文件的长度
