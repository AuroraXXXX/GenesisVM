...
#define SEGMENT_FORMAT               \
    "Segment-" SEGMENT_LV_FORMAT "%c@" PTR_FORMAT "[" PTR_FORMAT  ","  PTR_FORMAT ")"
...
namespace metaspace {
    ...
    void *Segment::allocate(size_t request_bytes) {
        ...
        auto used_top = this->_base_ptr + this->_used_bytes;
        this->_used_bytes += request_bytes;
        return (void *) used_top;

    }

    void Segment::initialize(Volume *container, void *base, SegmentLevel_t level) {
        this->_used_bytes = this->_committed_bytes = 0;
        this->_base_ptr = (uintptr_t) base;
        this->_level = level;
    ...
    bool Segment::commit_up_to(size_t new_commit_bytes) {
        ...
        auto res = this->ensure_range_is_committed((void *) (this->_base_ptr + commit_from),
    ...
    void Segment::clear_committed() {
        ...
        const auto used_bytes = this->used_bytes();
        const auto granule_bytes = Setting::CommitGranuleBytes;
        assert(this->state() == State::Free &&
               used_bytes == 0 &&
               total_bytes >= granule_bytes,
               "仅仅空闲块且尺寸大于提交粒度才允许撤销提交");

        if (total_bytes >= granule_bytes) {
            this->container()->uncommit_range((void *) this->_base_ptr, total_bytes);
    ...
    bool Segment::ensure_committed_enough(size_t bytes) {
        ...
        if (bytes >= this->free_below_committed_bytes()) {
            return this->commit_up_to(bytes);
        }
        return true;
    }
    ...
    bool Segment::is_continuous(Segment *segment) const {
        if (segment == nullptr){
            return false;
        }
        auto this_start = this->_base_ptr;
        auto this_end = this->_base_ptr + this->_used_bytes;
