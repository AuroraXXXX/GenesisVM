...
namespace metaspace {
    ...
    Segment *SegmentHeaderPool::alloc_segment_header() {
        /**
         * 首先从_dead_chunk中获取
         */
        ...
        assert(chunk_head == nullptr || chunk_head->state() == Segment::State::Dead, "错误");
        ...
        assert(chunk_head->state() == Segment::State::Dead,"ChunkHeader状态设置错误");
        this->_used_headers++;
        //将内存块头部的状态设置为空闲
        chunk_head->set_state(Segment::State::Free);
        return chunk_head;
    }

    void SegmentHeaderPool::free_segment_header(Segment *chunk) {
        assert(chunk!= nullptr&&chunk->state() == Segment::State::Free,"错误");
        chunk->set_state(Segment::State::Dead);
